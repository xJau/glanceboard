...
instead of a 6-colour Waveshare frame: portrait, 16 grays, native resolution.
...
### Rendering: band compositor, text always drawn by PIL

The canvas is split into declared regions — header, agenda, illustration band,
footer. The deterministic renderer draws all the text, always. Phase 2 fills the
illustration band and nothing else.
...
| Too many events | Rows compact (padding, then end times) before any event is dropped; then `+N altri` |
| Device offline at a slot | Next wake-up fetches the current board |

## Testing

73 tests, no network. Fixtures: a sample `.ics` carrying deliberate client data
...
render determinism, that nothing is drawn into the reserved band, atomic-write
behaviour, and every authentication path including the absence of a config
endpoint.

## Phase 2, and what this design already owes it

- `Board.to_dict()` is the payload; it is already safe.
- The illustration band already has coordinates.
